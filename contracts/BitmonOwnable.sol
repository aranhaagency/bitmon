...
contract BitmonOwnable is BitmonIndexes {
    ...
    function balanceOf(address _owner) external view returns (uint256) {
        ...
        return _bitmonOwnershipIndex[_owner].length;
    }

    // ownerOf returns the address owner of a tokenID
    function ownerOf(uint256 _tokenId) external view returns (address) {
        address owner = _bitmonIndexOwner[_tokenId];
    ...
    function tokensOfOwner(address _owner) external view returns (uint256[] memory) {
        require(_owner != address(0), "ERC721: owner query for zero account");
        return _bitmonOwnershipIndex[_owner];
    }
    ...
    function transfer(uint256 _tokenID, address _to) external  {
        require(_bitmonIndexOwner[_tokenID] == msg.sender, "ERC721: the sender is not owner of the token");
